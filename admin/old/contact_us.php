...
{
	case "":
		$sql = "select * from ".$tblMainPages." where autoid= 7";
	...
	$tmp_name = $_FILES["uploadimage"]["tmp_name"];
		...
		$image = "contact_us.".image_valid($_FILES["uploadimage"]["type"]);
		
		if (!empty($tmp_name)) {			
			move_uploaded_file($tmp_name, $uploaddir.$image);
			smart_resize_image($uploaddir.$image, 948, 195, true); 
			
			$sql = "UPDATE ".$tblMainPages." SET image = '".$image."' where autoid =7";
  ...
  <tr>
    <td width="972" height="415" align="center" valign="top"><form method="post" enctype="multipart/form-data" name="form" id="form">
      <table width="900" border="0">
        ...
        <tr>
          <td colspan="2" class="heading">Edit Contact Us</td>
          </tr>
        <tr>
        ...
        <!--<tr>
          ...
          ( width <strong>948px</strong> X <strong>195px</strong>)</td>
        </tr>-->
        <tr>
