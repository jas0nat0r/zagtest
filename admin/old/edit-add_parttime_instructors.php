...
switch ($Submit)
{
	...
	case "":
		$sql = "select * from ".$tblPartTimeInstructor." where autoid=".tosql($s_id,"Number");
	...
	case "Update":
		/*$sql = "UPDATE ".$tblPartTimeInstructor." SET instructor = '".forTexEditor($instructor)."',primary_number = '".forTexEditor($primary_number)."',secondary_number = '".forTexEditor($secondary_number)."',instruction = '".forTexEditor($instruction)."',email = '".forTexEditor($email)."',rates = '".forTexEditor($rates)."' where autoid=".$id;
...
<form method="post" enctype="multipart/form-data" name="formp" id="formp">
  <table width="850" border="0" align="left">
        <tr>
          <td colspan="2" class="heading"> Edit Part time Instructors</td>
          </tr>
        <tr>
