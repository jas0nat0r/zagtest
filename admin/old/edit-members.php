...
switch ($Submit)
{
	...
	case "":
		$sql = "select * from ".$tblMembers." where autoid=".tosql($m_id,"Number");
	...
	case "Update":
		...
		",s_description = ".tosql(forTexEditor($s_description),"Text").
		",page_url = ".tosql(forTexEditor($page_url),"Text").
		",target = ".tosql(forTexEditor($target),"Text").
		" WHERE autoid=".tosql($m_id,"Number");
...
<form method="post" enctype="multipart/form-data" name="formp" id="formp">
  <table width="850" border="0" align="left">
        <tr>
          <td colspan="2" class="heading"> Edit Member</td>
          </tr>
        <tr>
        ...
        <tr>
          <td valign="top" class="Normal">Member Image</td>
          <td class="links"><input name="uploadimage" type="file" class="NormalTextBox" id="uploadimage">
          (.png width <strong>265px</strong> X <strong>79px</strong>)</td>
        </tr>
  ...
  <input name="title" type="text" class="inputBox" id="title" style="width:500px" value="<?php echo(stripslashes(stripslashes($title)));?>"></td>
        ...
        <tr>
          <td align="left">&nbsp;</td>
      <td align="left"><img src="../images/members/<?=$image?>" width="265" height="79" /></td>
        </tr>
      </table>
</form>
</body>
