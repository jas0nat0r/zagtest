...
{
	case "":
		$sql = "select * from ".$tblSubPages." where autoid= 3";
	...
	$tmp_name = $_FILES["uploadimage"]["tmp_name"];
		...
		$image = "flight_simulators.".image_valid($_FILES["uploadimage"]["type"]);
		
		if (!empty($tmp_name)) {			
			move_uploaded_file($tmp_name, $uploaddir.$image);
			smart_resize_image($uploaddir.$image, 950, 305, true); 
			
			$sql = "UPDATE ".$tblSubPages." SET image = '".$image."' where autoid =3";
		...
		" WHERE autoid=".tosql(3,"Number");
		mysql_query($sql) or die (mysql_error());
		
		header("Location:flight_simulators.php");
		exit;
	break;
}
  ...
  <tr>
    <td width="972" height="415" align="center" valign="top"><form method="post" enctype="multipart/form-data" name="form" id="form">
      <table width="900" border="0">
        ...
        <tr>
          <td colspan="3" class="heading">Edit Flight Simulators</td>
          </tr>
        <tr>
          <td width="143" valign="top">&nbsp;</td>
          <td width="209" class="NormalRedSmall"><?=$err?>&nbsp;</td>
          <td width="534" class="linksPage"><a href="add_flight_simulators.php">Add Flight Simulators</a></td>
        </tr>
        <tr>
          <td valign="top" class="Normal">Page Image</td>
          <td colspan="2" class="links"><input name="uploadimage" type="file" class="NormalTextBox" id="uploadimage">
        ...
        <tr>
          ...
          <td colspan="2"><input name="title" type="text" class="NormalTextBox" id="title" value="<?=$title?>" size="75"></td>
        </tr>
        <tr>
        ...
        <tr>
          <td>&nbsp;</td>
          <td colspan="2"><img src="../assets/pages/<?=$image?>" width="745" height="238" /></td>
        </tr>
      </table>
    </form></td>
  </tr>
