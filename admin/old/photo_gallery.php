...
switch ($Submit)
{
	...
	case "delete":
		...
		$sql = "select images from ".$tblGallery." Where id=".$autoid;
		$rs = mysql_query($sql) or die(mysql_error());
		$row = mysql_fetch_assoc($rs);
		$images = $row['images'];
		
		delete_file("../images/gallery/".$images);
		delete_file("../images/gallery/t_".$images);
		
		$sql= "DELETE FROM ".$tblGallery." WHERE id=".tosql($autoid,"Number");
...
function Delete(autoid)
{
	...
	if (userreq==true)
	{
		//var url='photo_gallery.php?Submit=delete&autoid='+autoid+'&cid=<?php echo $cid ?>';
		var url='photo_gallery.php?Submit=delete&autoid='+autoid;
		window.location = url;
	}
}
</script>
...
$(document).ready(function() {
	...
	$("#uploadify").uploadify({
		...
		'script'         : 'photo_gallery_upload.php',
		'scriptData'	 : {'param': '1'},
