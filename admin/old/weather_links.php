...
{
	case "":
		$sql = "select * from ".$tblSubPages." where autoid= 8";
	...
	$tmp_name = $_FILES["uploadimage"]["tmp_name"];
		...
		$image = "weather_links.".image_valid($_FILES["uploadimage"]["type"]);
		
		if (!empty($tmp_name)) {			
			move_uploaded_file($tmp_name, $uploaddir.$image);
			smart_resize_image($uploaddir.$image, 950, 305, true); 
			
			$sql = "UPDATE ".$tblSubPages." SET image = '".$image."' where autoid =8";
		...
		" WHERE autoid=".tosql(8,"Number");
		mysql_query($sql) or die (mysql_error());
		
		header("Location:weather_links.php");
		exit;
	break;
}
  ...
  <tr>
    <td width="972" height="415" align="center" valign="top"><form method="post" enctype="multipart/form-data" name="form" id="form">
      <table width="900" border="0">
        ...
        <tr>
          <td colspan="2" class="heading">Edit Weather Links</td>
          </tr>
        <tr>
