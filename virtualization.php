...
$c_content = $rowc['content'];
  	...
  	$sqls = "select * from " . $tblServices . " WHERE id=4";
    $rss = mysql_query($sqls) or die(mysql_error());
    $rows = mysql_fetch_array($rss);
	
		$sqlc = "select * from " . $tblServices . " where id = 4 and status=1";
		$rsc = mysql_query($sqlc) or die(mysql_error());
		$rowc = mysql_fetch_assoc($rsc);
		$c_content = $rowc['content'];
...
<div class="middlesection innerbackground">
   <h2><?= $rows['title']; ?></h2>
    ...
    <div class="detailsleft">
    	<img src="images/pageservimg2.jpg" alt="detailimg"  class="solutionimage"/>
    </div>
